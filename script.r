...
dir.create("figures/hmd/population/", recursive = TRUE)
dir.create("figures/hmd/lmort/", recursive =TRUE)
dir.create("figures/hmd/mort/", recursive =TRUE)
...
spool_pop_figs <- function(x){
  ...
  file_label <- paste0(
    "population_", this_country, "_(", min_year, "_", max_year, ")" 
  )
  
  png(paste0("figures/hmd/population/", file_label, ".png"),
      res=300, width=40, height=20, units = "cm"
  )
  
  
  
  p <- x %>% filter( age <= 90 & sex !="total") %>% 
  ...
  contourplot(
    ...
    cex=1.4,
    cuts=20,
    col.regions=colorRampPalette(brewer.pal(6, "Blues"))(200),
    main=NULL,
    labels=list(cex=1.2),
    col="darkgreen",
    scales=list(
...
d_ply(dta_hmd, .(country), spool_pop_figs)


spool_mort_figs <- function(x){
  this_country <- x$country[1]
  min_year <- min(x$year)
  max_year <- max(x$year)
  ...
  file_label <- paste0(
    "mort_", this_country, "_(", min_year, "_", max_year, ")" 
  )
  
  png(paste0("figures/hmd/mort/", file_label, ".png"),
      res=300, width=40, height=20, units = "cm"
  )
  ...
  p <- x %>% filter( age <= 90 & sex !="total") %>% 
    mutate(mort_rate = death_count/ population_count) %>% 
    contourplot(
      mort_rate ~ year * age | sex, 
      data=. , 
      region=T, 
      par.strip.text=list(cex=1.4, fontface="bold"),
      ylab=list(label="Age in years", cex=1.4),
...
d_ply(dta_hmd, .(country), spool_mort_figs)





spool_lmort_figs <- function(x){
  this_country <- x$country[1]
  min_year <- min(x$year)
  max_year <- max(x$year)
  ...
  file_label <- paste0(
    "lmort_", this_country, "_(", min_year, "_", max_year, ")" 
  )
  
  png(paste0("figures/hmd/lmort/", file_label, ".png"),
      res=300, width=40, height=20, units = "cm"
  )
...
d_ply(dta_hmd, .(country), spool_lmort_figs)




dir.create("figures/hfd/asfr/", recursive = TRUE)
dir.create("figures/hfd/cpfr/", recursive = TRUE)
...
d_ply(dta_hfd, .(code), spool_asfr_figs, .progress="text")
